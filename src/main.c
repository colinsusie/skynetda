...
static void init_lua_path(lua_State *dL) {
    ...
    lua_pushfstring(dL, "%s;../?.lua;../?.luac", lua_tostring(dL, -2)); // [pkg|path|pathkey|pathval]
    lua_settable(dL, -4);    // [pkg|path]
    lua_pop(dL, 1); // [pkg]

    lua_getfield(dL, -1, "cpath");    // [pkg|cpath]
    lua_pushstring(dL, "cpath");     // [pkg|cpath|cpathkey]
    lua_pushfstring(dL, "%s;./?.so", lua_tostring(dL, -2)); // [pkg|cpath|cpathkey|cpathval]
    lua_settable(dL, -4);    // [pkg|path]
    lua_pop(dL, 2); // []
}

static bool run_script(lua_State *L) {
    int err = (luaL_loadfile(L, "../debugger.lua") || lua_pcall(L, 0, 4, 0));
    if (err) {
        fprintf(logger, "%s\n", lua_tostring(L, -1));
        return false;
    }
